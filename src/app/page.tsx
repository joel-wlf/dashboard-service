...
export default function Home() {
  ...
  const zipCode =
    settings.find((s) => s.key === "zip_code")?.value ||
  ...
  const testBedData =
    settings.find((s) => s.key === "testbed_info")?.value || [];
      ...
      >
        <div className='col-span-6 flex flex-col'>
          ...
          <div className='h-[90%] border-y border-gray-700 flex flex-col justify-center items-center'>
            {showClock && (
              <>
                <p className='text-[128px] font-mono text-white translate-y-2'>
                  {time.toLocaleTimeString("de")}
                </p>
                <p className='text-[32px] font-mono text-white -translate-y-2'>
                  {format(time, "EEEE, dd. MMMM yyyy", { locale: de })}
                </p>
              </>
            )}
          </div>
